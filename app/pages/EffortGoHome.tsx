...
export function EffortGoHome() {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-transparent text-foreground">
      <main className={`${contentShellClass} flex flex-1 flex-col pb-8 pt-4`}>
        <section className="grid gap-10 md:py-2 lg:grid-cols-[minmax(0,1fr)_minmax(380px,440px)] lg:items-start">
          <div className="flex max-w-none flex-col items-center text-center lg:items-start lg:text-left">
            <p className="mb-4 inline-flex items-center gap-2 rounded-lg border border-border bg-muted px-3 py-1 text-sm text-primary">
              <Sparkles className="size-4 text-primary" />
              {home.eyebrow}
            </p>
            <h1 className="text-4xl font-semibold leading-tight tracking-normal lg:whitespace-nowrap">
              {home.slogan}
            </h1>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:w-full lg:max-w-[440px] lg:justify-self-end lg:grid-cols-1">
            {home.highlights.map((item) => (
              <article key={item.title} className="rounded-lg border border-border bg-card p-4 shadow-sm backdrop-blur-xl">
            ...
            {home.apps.map((app) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex min-h-48 flex-col rounded-lg border border-border bg-card p-5 shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-primary/50 hover:bg-muted/70 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/40"
                >
                  <div className="mb-5 flex items-start justify-between gap-3">
                    ...
                    <span className="flex flex-col items-end gap-2">
                      ...
                      <span className="rounded-md border border-border px-2 py-0.5 text-[0.68rem] font-semibold uppercase tracking-normal text-muted-foreground transition group-hover:border-primary/40 group-hover:text-primary">
                        {category}
                      </span>
                    </span>
                  </div>
